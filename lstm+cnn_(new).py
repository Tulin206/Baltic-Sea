...
Original file is located at
    https://colab.research.google.com/drive/1V0eKhX1PRiIJfv7oHCLSAA2jDAjMTO0B
"""
...
# Plot histograms for temperature and salinity (ocean-only values)
plt.figure(figsize=(12, 5))

# Temperature histogram
plt.subplot(1, 2, 1)
plt.hist(temperature_data_non_nan, bins=50, color='skyblue')
plt.title("Temperature Data Distribution (Ocean Areas)")
plt.xlabel("Temperature")
plt.ylabel("Frequency")

# Salinity histogram
plt.subplot(1, 2, 2)
plt.hist(salinity_data_non_nan, bins=50, color='salmon')
plt.title("Salinity Data Distribution (Ocean Areas)")
plt.xlabel("Salinity")
plt.ylabel("Frequency")
...
print("train shape:", sample.shape)  # Should be (num_samples, look_back, c, w, h)
...
# Create sample pairs for both training and validation sets
X_train_original, y_train_original = create_sample_pairs(train_data, look_back, look_ahead, step_size)
X_val_original, y_val_original = create_sample_pairs(val_data, look_back, look_ahead, step_size)

# Reshape X_train to the correct input shape expected by the model
X_train = np.transpose(X_train_original, (0, 1, 3, 4, 2))  # Reshape to (samples, time_steps, height, width, channels)
X_val = np.transpose(X_val_original, (0, 1, 3, 4, 2))      # Same for validation data
y_train = np.transpose(y_train_original, (0, 2, 3, 1))
y_val = np.transpose(y_val_original, (0, 2, 3, 1))
...
look_back_data = X_train_original[random_i, :, :, random_w, random_h]
print("look_back_data shape:", np.shape(look_back_data))
print(look_back_data.shape)
look_ahead_data = y_train_original[random_i, :, random_w, random_h]
...
# Data for look-back and look-ahead
look_back_data = X_val_original[random_i, :, :, random_w, random_h]
print("look_back_data shape:", np.shape(look_back_data))
print(look_back_data.shape)
look_ahead_data = y_val_original[random_i, :, random_w, random_h]
print("look_ahead_data shape:", np.shape(look_ahead_data))
print(look_ahead_data.shape)
...
channels = X_train.shape[4]    # 2 (temperature, salinity)
width = X_train.shape[2]       # 111
height = X_train.shape[3]      # 251

# Define the model
#input_layer = Input(shape=(time_steps, channels, height, width))
input_layer = Input(shape=(time_steps, height, width, channels))  # Correct input shape
...
temperature_output = Reshape((height, width, 1), name='temperature_output')(temperature_output)  # 1 channel for temperature
...
salinity_output = Reshape((height, width, 1), name='salinity_output')(salinity_output)  # 1 channel for salinity
...
print(f"y_val_temperature shape: {y_val_temperature.shape}")
print(f"y_val_salinity shape: {y_val_salinity.shape}")
...
'''
batch_size = 8  # Use the same batch size you used during training
...
for i in range(0, len(X_val), batch_size):
    ...
    # Store the predictions for temperature and salinity
    y_pred_temperature.append(y_pred_batch[0])
    y_pred_salinity.append(y_pred_batch[1])

# Convert lists to arrays
y_pred_temperature = np.concatenate(y_pred_temperature, axis=0)
y_pred_salinity = np.concatenate(y_pred_salinity, axis=0)
'''
...
print("Flattened Predicted Temperature Shape:", temperature_pred_flat.shape)
print("Flattened Predicted Salinity Shape:", salinity_pred_flat.shape)
print("Flattened True Temperature Shape:", y_val_temperature_flat.shape)
print("Flattened True Salinity Shape:", y_val_salinity_flat.shape)
...
# Define custom SMAPE function
def smape(y_true, y_pred):
    # Calculate SMAPE with a small constant to avoid division by zero
    return 100 * np.mean(2 * np.abs(y_pred - y_true) / (np.abs(y_true) + np.abs(y_pred) + 1e-10))

# Calculate SMAPE
smape_temperature = smape(y_val_temperature_flat, temperature_pred_flat)
smape_salinity = smape(y_val_salinity_flat, salinity_pred_flat)

# Print the SMAPE scores
print("\nSMAPE (Symmetric Mean Absolute Percentage Error)")
print(f"Validation Temperature: {smape_temperature:.4f}%")
print(f"Validation Salinity: {smape_salinity:.4f}%")
